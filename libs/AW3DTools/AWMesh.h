...
public:
   ...
   int            addIndex(const unsigned int i);
   ...
   int            getIndex(int i)         {return m_indeces[i];}
   ...
   unsigned int*  getIndexList()          {return m_indeces.getList();}
...
//MEMBER DATA
   ...
   AWList<unsigned int>       m_indeces;  //face indeces
...
public:
   ...
   AWMesh*			calcMesh(float t, AWMesh* buf=NULL);
...
inline int            
AWMesh::addIndex(const unsigned int i)
{
   return m_indeces.add(i);
}//inline int AWMesh::addIndex(const unsigned int i)
