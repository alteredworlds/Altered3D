...
AWDEFINE_VERSIONABLE_PERSISTABLE(AWNode, CLSID_AWNODE, VER_AWNODE_4);


AWNode::AWNode(const AWCString& name, AWNode* parent, float life) :
	m_transformer(NULL),
	m_name(name),
	m_children(3),
	m_parent(parent),
...
int	 	 
AWNode::calcTransform(float t, const AWMatrix4& parentTM)
{
   int retVal = 0;
   m_wsTM = parentTM;
   float useTime = t - m_animStart;
   ...
   return retVal;
}//int AWNode::calcTransform(float t, const AWMatrix4& parentTM)
