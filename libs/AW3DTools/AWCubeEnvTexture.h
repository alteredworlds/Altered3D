...
public:
   ...
   AWBoolean            needsUpdate(float t);
   void                 setLastUpdate(float t);
...
inline AWBoolean            
AWCubeEnvTexture::needsUpdate(float t)
{
   return (-1==m_step) ? FALSE : (0.0f == m_lastUpdate) ? TRUE : (t-m_lastUpdate)>m_step;
}//inline AWBoolean AWCubeEnvTexture::needsUpdate(float t)



inline void
AWCubeEnvTexture::setLastUpdate(float t)
{
   m_lastUpdate = t;
   if (m_step==0.0f)
