...
int						
AWTimerBarController::calcTransform(float t, const AWMatrix4& parentTM)
{
    float fac = m_timeout - t;
    AWPoint useScale(getScale());
    if ((fac >= 0.0f) && (m_timeout > 0.0f))
    {
...
}//int AWTimerBarController::calcTransform(float t, const AWMatrix4& parentTM)
