...
public:
//General Properties
   virtual AWBoolean isAlive(float time);
   AWBoolean		   isAnimated();
   void		         setAnimStartTime(float t, AWBoolean andKids=TRUE);
   float			      getAnimStartTime()		{return m_animStart;}
...
//indicate ownership of children and transformer
	void			      setOwnsChildren(AWBoolean val) 
										{if (val) m_ownership |= AW_NODE_OWNS_CHILDREN; 
										else m_ownership &= NOT_AW_NODE_OWNS_CHILDREN;}
   ...
   void              setLastUpdate(float t, AWBoolean andKids=TRUE);
...
//manipulation of CHILDREN
	...
	float*			   getMatrix() 
						   {return (m_transformer) ? m_transformer->getMatrix() : m_wsTM.m_mat;}
...
//MEMBER DATA associated with CHILDREN
    ...
    float                  m_animStart;    //not persisted
...
inline void    
AWNode::init(float t, GLRenderer& renderer)
{
...
inline int	 	 
AWNode::calcViewDependentData(float t, const AWSolidCone& viewCone)
{
	...
	return 1;
}//int AWNode::calcViewDependentData(float t, const AWSolidCone& viewCone)



inline int		   
AWNode::calcVisibility(float t, const AWSolidCone& viewCone)
{  //iterate through children and calcViewDependentData
	for (int i = 0; i < m_children.getNum(); i++)
	{
...
inline void				
AWNode::setAnimStartTime(float t, AWBoolean andKids)
{
	m_animStart = t;
	if (andKids)
	{
...
inline void				
AWNode::setLastUpdate(float t, AWBoolean andKids)
{
	if (m_transformer)
    {
...
AWNode::getViewCone(AWSolidCone& viewCone, AWMatrix4& wsTM)
{
   ...
   //hencewe need to invert the transformed Z axis direction 
