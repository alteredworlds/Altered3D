...
AWDEFINE_VERSIONABLE_PERSISTABLE(AWFlock, CLSID_AWFLOCK, VER_AWFlock_4);


#ifdef _WIN32
float rnd() {return (float)rand()/(float)RAND_MAX;}
#else
float rnd() {return (float)random()/(float)RAND_MAX;}
#endif
...
int 
AWFlock::populate(float t, GLRenderer& renderer)
{
...
void
AWFlock::init(float t, GLRenderer& renderer)
{
   m_renderer = &renderer;
   //need to know number of boids, and object to be used as clone source
   //for now, assume that m_numBoids is valid
   setAnimStartTime(t, FALSE);
   setLastUpdate(0.0f);
   if(getEnabled() && m_renderer)
   {
      m_renderer->initTimeBar(getTimeout());
   }
   ...
   {	
      ...
      for (int i = 0; i < getNumChildren(); i++)
      {
         ...
         nextBoid->setAnimStartTime(rnd() - 0.5f, FALSE);
...
int		
AWFlock::calcTransform(float time, const AWMatrix4& parentTM)
{ 
    float useTime = time - m_animStart;
	...
	return 1;
}//int AWFlock::calcTransform(float t, const AWMatrix4& parentTM)
