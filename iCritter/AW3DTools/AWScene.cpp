...
#define VER_AWScene_9 108

AWDEFINE_VERSIONABLE_PERSISTABLE(AWScene, CLSID_AWSCENE, VER_AWScene_9);
...
}//void AWScene::calcTextures(double t)

void              
AWScene::update(float viewZoom, double time)
{
   m_nodeList.updateTransformer(time);
	m_nodeList.setZoom(viewZoom);
...
{	
   ...
   file >> m_landscape;
    if (version >= VER_AWScene_9)
    {
        file >> m_loopTime;
    }
    else if (version >= VER_AWScene_2)
    {
        float tmp;
        file >> tmp;
        m_loopTime = (double)tmp;
    }
