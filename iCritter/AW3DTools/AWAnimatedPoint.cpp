...
void
AWAnimatedPoint::dumpKeys()
{
   if (m_pointList.getNum())
   {
      AWPositionKey* key = NULL;
      AWPoint        p;
      AWTRACE1("AWAnimatedPoint::dumpKeys() - %i keys\n", (int)m_pointList.getNum());
      for (int i=0; i<m_pointList.getNum(); i++)
      {
         key  = (AWPositionKey*)m_pointList[i];
         if (key)
         {
            p = key->m_pos;
            AWTRACE3("Processed Point(x,y,z) (%f, %f, %f) ", 
                     (double)p.x, (double)p.y, (double)p.z);
            AWTRACE1(" at %f\n", (double)key->m_t);
         }//if (key)
      }
   }
}//void AWAnimatedPoint::dumpKeys()
