...
public:
   ...
   AWObject(const AWCString& name = "", AWNode* parent=NULL, 
	...
	void				calcMesh(double time);
    void                calcViewDependentMesh(double time, const AWSolidCone& viewCone);
	void				setAnimation(int animNo);

	//update the object state with the values for the specified time
	virtual int			calcTransform(double time, const AWMatrix4& parentTM);
	//update the object state with the view-dependent values at t
	virtual int			calcViewDependentData(double time, const AWSolidCone& viewCone);
   
   //update the view-dependent visibility value at t
	virtual int         calcVisibility(double time, const AWSolidCone& viewCone);
...
//and these do not, might possibly be quicker
  ...
  // const float*		getTexCoordfv(int i, int tUnit=0); 
   ...
   unsigned short*     getIndexArray() {return m_curMesh?m_curMesh->getIndexList():NULL;}
   ...
   void	            checkCurMesh();
	...
	virtual AWBoolean	collideWith(AWObject* target, double time=0.0);
...
inline int			
AWObject::calcTransform(double t, const AWMatrix4& parentTM)
{
