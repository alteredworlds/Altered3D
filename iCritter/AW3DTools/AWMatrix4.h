...
public:
	...
	void			awRotateX(float angle, bool degress=true);
	void			awRotateY(float angle, bool degress=true);
	void			awRotateZ(float angle, bool degress=true);
	
	void			frustum(float l, float r, float b, float t,
							float zNear, float zFar);
	void			frustumInverse(float l, float r, float b, float t, 
							 float zNear, float zFar);
    void            perspectiveAlt(float fovy, float aspect, float zNear, float zFar);
	...
	void			lookAt(const AWPoint& forward, const AWPoint& upVector3D);
...
inline void 
AWMatrix4::perspectiveAlt(float fovy, float aspect, float zNear, float zFar)
{
    identity();
    //
	...
	m_mat[8] = 0.0f;
	m_mat[9] = 0.0f;
	m_mat[10] = (zFar+zNear) / (zNear-zFar);
	m_mat[11] = -1.0f;
	
	m_mat[12] = 0.0f;
	m_mat[13] = 0.0f;
	m_mat[14] = 2 * zFar * zNear /  (zNear-zFar);
	m_mat[15] = 0.0f;
}
