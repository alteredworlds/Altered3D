...
#define VER_AWMesh_5 VER_AWMesh_4 + 1
#define VER_AWMesh_6 VER_AWMesh_5 + 1
...
AWDEFINE_VERSIONABLE_PERSISTABLE(AWMesh, CLSID_AWMESH, VER_AWMesh_6);
...
AWMesh*		 
AWAnimatedMesh::calcMesh(double t, AWMesh* buf)
{
	AWMesh* retVal = NULL;
	//should really -
   ...
   if (1==m_frames.getNum() && !buf)
   {
	...
	return retVal;
}//AWMesh* AWAnimatedMesh::calcMesh(double t,  AWMesh* buf)
...
//versions 2 and greater store directly
		if (version >= VER_AWMesh_6)
		{
			m_colVerts.read(file);
		}
		else if (version >= VER_AWMesh_2)
		{
			AWList<tmpColV> tmp;
         tmp.read(file);
			AWColVertex tColVert;
			tColVert.a = 255;
			for (int idx=0; idx<tmp.getNum(); idx++)
			{
				tColVert.x = tmp[idx].x;
				tColVert.y = tmp[idx].y;
				tColVert.z = tmp[idx].z;
      ...
      {  //ver 3 or higher...
         ...
         else
         {
			...
			if (version <= VER_AWMesh_5)
			{
				m_indeces.read(file); 
			}
			else
			{	// read in old format of AWList<unsigned int> and convert to new format
				AWList<unsigned int> tmp;
				tmp.read(file);
				for (int idx=0; idx<tmp.getNum(); idx++)
				{
					m_indeces.add((unsigned short)tmp[idx]);
				}
			}
      }//if (version >= VER_AWMesh_3)
