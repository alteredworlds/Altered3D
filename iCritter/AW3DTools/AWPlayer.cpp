...
AWPlayer::AWPlayer(AWBoolean openGL2, AWBoolean useLights, AWBoolean useFog, AWBoolean useVertexColours)
    : m_initCalled(FALSE)
{
    ...
    // want to turn on / off fog
    m_renderer->setUserAllowsFog(useFog);
    // want to turn on / off lights
    m_renderer->setUserAllowsLighting(useLights);
    // want to turn on / off vertex colours
    m_renderer->setUserAllowsVertexColours(useVertexColours);
}
