...
public:
	...
	int		     populate(double t, AWGLRenderer& renderer);	//populate flock with clones, returns num added
	virtual void init(double t, AWGLRenderer& renderer);		//initialise the clone values
    virtual void reset(double t);    //put back to state before populate
    ...
    void        setTimeout(double v)             {m_timeout=v;}
...
protected:
	...
	float		m_minY;
	float		m_maxY;
    double       m_timeout;      //really just for savergames...
    AWPoint     m_startPosn;
