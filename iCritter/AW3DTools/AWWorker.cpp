...
void
AWWorker::init(double time, AWGLRenderer& renderer)
...
void				 
AWWorker::startup(double t, AWGLRenderer& renderer)
{
   //calculate object face, radius data
   init(0.0, renderer);	//int all scene objects
...
float                
AWWorker::playTransition(double t, int i)
{
   float retVal = 0.0f;
...
void 
AWWorker::resetLevel(double time, AWGLRenderer& renderer)
{
    ...
    if (numFlocks && (m_flockIndex < numFlocks) && (m_flockIndex >= 0))
    {
        ...
        useFlock->populate(time, renderer);  //also removes existing children
        useFlock->init(time, renderer);
	    //and add all the generated child (flock member) objects to the display list
	    for (child=0; child<useFlock->getNumChildren(); child++)
	    {
...
void
AWWorker::initFlock(AWFlock* flock, AWGLRenderer& renderer)
{
	if (flock)
	{
...
void
AWWorker::initTimeBar(double timeout, double timeNow)
{
    if (m_timerBarCtl)
    {
...
void
AWWorker::resetTimeBar(double timeNow)
{
    if (m_timerBarCtl)
    {
        m_timerBarCtl->init(timeNow);
...
void		
AWWorker::updateTransformer(double t)
{
   AWTRACEHI("AWWorker::updateTransformer");
   if(m_userTransform) 
   {
      AWTRACEHI1(" m_userTransform assigned - update at %f", t);
      m_userTransform->updateTransform(t);	
   }
   AWTRACEHI("\n");
}
...
AWBoolean
AWWorker::collideObjectWithWorld(AWObject* srce, double time)
...
void							
AWWorker::collideAllObjects(double time)
...
void
AWWorker::processEvents(AWNode& n, double t)
...
void							
AWWorker::transformAllNodes(double t)
{
   AWMatrix4 tmp;
	tmp.identity();
	//call virtual void AWNode::calcTransform(double t, const AWMatrix4& parentTM)
...
void 
AWWorker::calcVisibility(double t)
{
   if (m_curCamera) m_curCamera->getViewCone(m_viewCone);
...
void 
AWWorker::calcVisibility(double t, AWSolidCone& viewCone)
{
