...
// - QMDX_GetChannelParams
// - QMDX_SetChannelParams
// - QMDX_CalculateChannelParams
// - QMDX_FindChannel
// - QMDX_PlayEx
//
// The first set of parameters shouldn't be modified, the second set
// can be altered for QMDX_SetChannelParams, and the third set
...
struct QMIX_CHANNEL_PARAMS {
    ...
    DWORD dwModes;                  // combination of QMDX_EnableChannel flags
...
// QMDX_FindChannel until a sound is played on the channel.  This is
...
//
// Structure describing wave data; returned by QMDX_OpenWave and
// QMDX_OpenWaveEx.
//
...
//
// QMDX_OpenWave and QMDX_OpenWaveEx flags.
//
...
//
// Options for dwFlags parameter in QMDX_PlayEx.
//
...
// until the next sound is played with QMDX_PlayEx.  This allows
...
//   - QMDX_EnableChannel
//   - QMDX_SetChannelParams
//   - QMDX_SetChannelPriority
//   - QMDX_SetDistanceMapping
//   - QMDX_SetFrequency
//   - QMDX_SetPan
//   - QMDX_SetPanRate
//   - QMDX_SetPolarPosition
//   - QMDX_SetPosition
//   - QMDX_SetSourceCone
//   - QMDX_SetSourceCone2
//   - QMDX_SetSourcePosition
//   - QMDX_SetSourceVelocity
//   - QMDX_SetVolume
//
...
// out changes.
//
// Use QMDX_SetPanRate(hQMixer, iChannel, QMIX_WAIT|QMIX_USEONCE, 0)
// to implement sudden changes for the next sound without losing the
// smoothing function.  Using QMIX_IMMEDIATE with QMDX_PlayEx also
...
//
// QMDX_EnableChannel:  if mode==0, use conventional, high-performance
...
//
// QMDX_Enable is equivalent to
//   QMDX_EnableChannel(hQMixer, 0, QMIX_ALL, mode).
//
_QMIXIMP
MMRESULT
WINAPI
...
//**********************************************************************
// Call QMDX_Pump to force more mixing.  Essential if mixer was
...
//
// Note that, in versions 2.10 and later, that QMDX_FreeWave can be
// called with a null hQMixer if QMDX_CloseSession has been called.
//
_QMIXIMP
MMRESULT
WINAPI
...
//
// Diagnostic codes available from QMDX_GetLastError.  Equivalent strings
// can be recalled using QMDX_GetErrorText.
//
...
// when QMDX_OpenWave is used with the QMIX_RESIDENT_COMPRESSED
// parameter.
//
...
typedef struct MIXPLAYPARAMS {
    ...
    HQMIXER hMixSession;    // value returned by QMDX_Init or QMDX_ConfigureInit
    int iChannel;           // channel number
    LPMIXWAVE lpMixWave;    // pointer returned by QMDX_OpenWave
    HWND hWndNotify;        // if set, WOM_OPEN and WOM_DONE notification messages sent here
    DWORD dwFlags;
